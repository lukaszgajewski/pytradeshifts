...
def test_compute_calories(oceania_dataset_filtered):
    ...
    assert "Calories" in trade_data.columns
    trade_data = trade_data.replace([np.inf, -np.inf], np.nan)
    assert ~trade_data.isna().any(axis=None)
    assert trade_data.dtypes["Calories"] == np.dtype("float64")
    assert (trade_data["Calories"] >= 0).all(axis=None)
    assert isinstance(production_data, pd.DataFrame)
    assert len(production_data.columns) == 3
    assert "Calories" in production_data.columns
    production_data = production_data.replace([np.inf, -np.inf], np.nan)
    assert ~production_data.isna().any(axis=None)
    assert production_data.dtypes["Calories"] == np.dtype("float64")
    assert (production_data["Calories"] >= 0).all(axis=None)
